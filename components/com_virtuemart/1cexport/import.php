...
{
	require_once(JPATH_BASE_1C .DS.'import_xml.php');
} 
elseif($_REQUEST['filename'] == 'offers.xml') 
{
	require_once(JPATH_BASE_1C .DS.'offers_xml.php');
} 
else 
{
...
if(defined( 'VM_SITE' ))
{
	...
	if ($_REQUEST['filename'] == 'import.xml')
	{
		$form = 'vmshop_1c.php?mode=import&filename=offers.xml';
		$button = '<input type="submit" value="Далее -> Выгрузка цен"'.$enable.'>';
	}
	elseif ($_REQUEST['filename'] == 'offers.xml')
	{
		$form = 'index.php';

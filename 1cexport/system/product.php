...
function createProduct($data='',$modif='false', $custom_id='0',$harakt='',$cvid,$cvzn) 
{
	...
	if(!empty($data['nds_name']) and $data['nds_name'] != '' and isset($data['nds_name']))
	{
		...
		$sql = "SELECT ".$dba['tax_rate_id_t']."  FROM #__".$dba['tax_rate_db']." where `".$dba['tax_rate_value_t']."` = '" . $nds_db . "'";
		$db->setQuery ( $sql );
	...
	$sql = "SELECT category_id FROM #__".$dba['category_to_1c_db']." where `c_category_id` = '" . $db->getEscaped($data['category_1c_id']) . "'";
	$db->setQuery ( $sql );
	$rows_sub_Count = $db->loadResult ();

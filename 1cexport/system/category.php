...
function inserCategory($xml, $parent = 0) 
{
	...
	{		
		...
		{ 
			...
			$db->setQuery ( $sql );
			JLog::add ( 'id= '.$id, JLog::DEBUG, 'vmshop_1c' );
			JLog::add ( 'sql= '.$sql, JLog::DEBUG, 'vmshop_1c' );
			...
			else 
			{
				...
				elseif(VM_VERVM == '2')
				{
					...
					$ins->ordering = '0';
					$ins->shared  = '0';
					$ins->published  = VM_PUBLISH_NEW_CATEGORY;    // SirPiter : публикация новой категории задается в config.php
